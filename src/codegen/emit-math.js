...
function emitMathAbs(dest, src) {
  ...
  this.instr('cqo');                     // rdx = extensao de sinal
  this.instr('xor rax, rdx');            // inverte bits se negativo
  this.instr('sub rax, rdx');            // soma 1 se era negativo
...
function emitMathMax(dest, left, right) {
  ...
  this.instr('cmovl rax, rcx');          // se left < right, usa right
  this.instr(`mov ${this.loc(dest)}, rax`);
}
...
function emitMathMin(dest, left, right) {
  ...
  this.instr(`mov rcx, ${this.loc(right)}`);
  this.instr('cmp rax, rcx');
  this.instr('cmovg rax, rcx');          // se left > right, usa right
  this.instr(`mov ${this.loc(dest)}, rax`);
}

function emitMathSqrt(dest, src) {
  // Converte inteiro para double, sqrt, armazena como double (TYPE_FLOAT)
